...
namespace QuanLyCafe.Presentation
{
    public partial class fr_Main : Form
    {
        ...
        {
            label_welcome.Text = "Xin chào " + username + "!";
            if (permission != "Admin")
            {
                nhânViênToolStripMenuItem.Dispose();
                nhàCungCấpToolStripMenuItem.Dispose();
                sảnPhẩmToolStripMenuItem.Dispose();
                hóaĐơnNhậpHàngToolStripMenuItem.Dispose();
                thốngKêToolStripMenuItem.Dispose();
                hóaĐơnNhậpToolStripMenuItem1.Dispose();
                tạoTàiKhoảnToolStripMenuItem.Dispose();
            }
        }

        private void càiĐặtMặtToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Form frm = kiemtratontai(typeof(FrmPrincipal));
            if (frm != null)
                frm.Activate();
            else
            {
                ...
                fr.MdiParent = this;
                fr.ReturnPermission = permission;
                fr.ReturnUserName = username;
                fr.ReturnMode = "Trainning";
                fr.Show();
            }
        }
        ...
        {
            Form frm = kiemtratontai(typeof(fr_reg));
            if (frm != null)
                frm.Activate();
            else
            {
                fr_reg fr = new fr_reg();
                fr.MdiParent = this;
                fr.Show();
            }
        }
