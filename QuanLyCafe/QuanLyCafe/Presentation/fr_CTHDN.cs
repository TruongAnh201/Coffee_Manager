...
namespace QuanLyCafe
{
    public partial class fr_CTHDN : Form
    {
        ...
        public void hienthi()
        {
            string sql = @"SELECT mahdn, tensp, tb_CTHDN.soluong, dongia, khuyenmai, thanhtien
                        FROM tb_CTHDN
        ...
        {
            if (cbhang.SelectedValue.ToString() != "")
            {
                if (themmoi == true)
                {
                    try
                    {
                        ...
                        ck.MAHDN = cbhd.Text;
                        ck.MASP = cbhang.SelectedValue.ToString();
                ...
                string upsl = "UPDATE tb_Sanpham SET soluong =soluong + '" + txtsl.Text + "' where masp='" + cbhang.SelectedValue.ToString() + "'";
                string upsl1 = "UPDATE tb_Sanpham SET gianhap ='" + txtdg.Text + "' where masp ='" + cbhang.SelectedValue.ToString() + "'";
                string upsl2 = "UPDATE tb_Sanpham SET giaban ='" + gb + "'where masp='" + cbhang.SelectedValue.ToString() + "'";
                string uptt = "update tb_HDN set tongtien=(SELECT sum(thanhtien) FROM tb_CTHDN where mahdn ='" + cbhd.Text + "') where mahdn='" + cbhd.Text + "'";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try { 
                ...
                exRange.Range["A1:B1"].Value = "Cafe Bất Ổn";
                exRange.Range["A2:B2"].MergeCells = true;
                exRange.Range["A2:B2"].HorizontalAlignment = COMExcel.XlHAlign.xlHAlignCenter;
                exRange.Range["A2:B2"].Value = "Việt Nam";
                exRange.Range["A3:B3"].MergeCells = true;
                exRange.Range["A3:B3"].HorizontalAlignment = COMExcel.XlHAlign.xlHAlignCenter;
                exRange.Range["A3:B3"].Value = "Điện thoại: 0999999999";
                ...
                sql = @"SELECT  tb_CTHDN.mahdn, tb_NCC.tenncc, tb_NCC.diachi, tb_NCC.sdt, tb_Nhanvien.tennv,tb_HDN.ngaynhap
                      FROM tb_HDN
                      INNER JOIN tb_NCC ON tb_HDN.mancc = tb_NCC.mancc 
                      INNER JOIN tb_CTHDN ON tb_HDN.mahdn = tb_CTHDN.mahdn 
                      INNER JOIN tb_Nhanvien ON tb_HDN.manv = tb_Nhanvien.manv 
                      where tb_HDN.mahdn='" + cbhd.Text + "'";
                ...
                sql = @"SELECT tb_Sanpham.tensp, tb_CTHDN.soluong, tb_CTHDN.dongia, tb_CTHDN.khuyenmai, tb_CTHDN.thanhtien
                      FROM tb_CTHDN 
                      INNER JOIN tb_Sanpham ON tb_CTHDN.masp = tb_Sanpham.masp AND tb_CTHDN.masp = tb_Sanpham.masp where tb_CTHDN.mahdn='" + cbhd.Text + "'";
                ...
                for (hang = 0; hang <= tblThongtinHang.Rows.Count - 1; hang++)
                {
                    ...
                    tongtien += float.Parse(tblThongtinHang.Rows[hang][4].ToString());
                }
                ...
                exBook.SaveAs(Application.StartupPath + "/Hóa Đơn Nhập.xlsx");
                exApp.Visible = true;
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
                MessageBox.Show("Vui lòng chọn đúng hàng có dữ liệu");
            }
}

        private void calc()
        {
            float sl = (txtsl != null && !string.IsNullOrWhiteSpace(txtsl.Text)) ?
                float.Parse(txtsl.Text) : 0;
            float dg = (txtdg != null && !string.IsNullOrWhiteSpace(txtdg.Text)) ?
                float.Parse(txtdg.Text) : 0;
            float gg = (txtgg != null && !string.IsNullOrWhiteSpace(txtgg.Text)) ?
                float.Parse(txtgg.Text) : 0;
            float t1 = sl * dg - gg;
