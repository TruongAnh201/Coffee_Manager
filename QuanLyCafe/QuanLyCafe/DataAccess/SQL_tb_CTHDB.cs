    ...
    class SQL_tb_CTHDB
    {
        ...
        public void themmoicthdb(EC_tb_CTHDB cthdb)
        {
            cn.ExcuteNonQuery(@"INSERT INTO tb_CTHDB
                      (mahdb, masp, soluong, thanhtien, khuyenmai) VALUES   (N'" + cthdb.MAHDB  + "',N'" + cthdb.MASP  + "',N'" + cthdb.SOLUONG  + "',N'" + cthdb.THANHTIEN + "',N'" + cthdb.KHUYENMAI + "')");
        }
        ...
        {
           cn.LoadLenCombobox(masp, "SELECT tensp, masp FROM tb_Sanpham");
        }
        public string Loadsoluong(string Soluong, string masp)
        {
            Soluong = cn.LoadLable("SELECT [soluong] From [tb_Sanpham] where masp= N'" + masp + "'");
            return Soluong;
        }
        //load THD
        public void loadmahd(ComboBox mahdb)
        {
            cn.LoadLenCombobox(mahdb, "SELECT sohdb FROM tb_HDB");
        }
        //load đơn giá bán
