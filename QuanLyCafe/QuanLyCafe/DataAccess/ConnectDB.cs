...
namespace QuanLyCafe.DataAccess
{
    class ConnectDB
    {
        ...
        public void LoadLenCombobox(ComboBox cb, string SQL)
        {
            con = getcon();
            ...
            {
                MessageBox.Show("Lỗi kết nối" + se.ToString());
            }
            finally
            {
                con.Close();
        ...
        public string GetType(string strsql)
        {
            con = getcon();
            con.Open();
            sqlcom = new SqlCommand(strsql, con);
            return (string)sqlcom.ExecuteScalar();
        }
    }
}
